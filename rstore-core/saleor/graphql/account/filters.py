...
from ...account.models import User, Thana, UserRequest,UserCorrectionRequest
...
from .enums import StaffMemberStatus, AgentRequestStatus ,UserCorrectionRequestStatus
...
def agent_request_filter_status(qs, _, value):
    qs = qs.filter(status=value)
    return qs


def user_correction_request_filter_status(qs, _, value):
    qs = qs.filter(status=value)
    return qs
...
def filter_staff_search(qs, _, value):
    search_fields = (
        ...
        "regions__thana__name",
        "regions__district__name"
    )
    if value:
...
def filter_agent_request_search(qs, _, value):
    search_fields = (
        "user__email",
        "user__first_name",
        "user__last_name",
...
def filter_user_correction_request_search(qs, _, value):
    search_fields = (
        "user__email",
        "user__first_name",
        "user__last_name",
...
class AgentRequestSearchFilter(django_filters.FilterSet):
    status = EnumFilter(input_class=AgentRequestStatus, method=agent_request_filter_status)
    search = django_filters.CharFilter(method=filter_agent_request_search)
...
class UserCorrectionRequestSearchFilter(django_filters.FilterSet):
    status = EnumFilter(input_class=AgentRequestStatus, method=user_correction_request_filter_status)
    search = django_filters.CharFilter(method=filter_user_correction_request_search)

    class Meta:
        model = UserCorrectionRequest
        fields = ["search"]
        fields = ["status","search"]
