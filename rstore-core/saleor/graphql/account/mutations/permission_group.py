...
class PermissionGroupCreate(ModelMutation):
    ...
    def save(cls, info, instance, cleaned_input):
        add_keycloak_group(instance.name)
    ...
    def _save_m2m(cls, info, instance, cleaned_data):
        ...
        if add_permissions:
            add_keycloak_group_permission(instance.name, add_permissions)
            instance.permissions.add(*add_permissions)

        users = cleaned_data.get("add_users")
        if users:
            add_keycloak_group_user(instance.name, users)
...
class PermissionGroupUpdate(PermissionGroupCreate):
    ...
    def _save_m2m(cls, info, instance, cleaned_data):
        ...
        if remove_users:
            remove_keycloak_group_user(instance.name, remove_users)
            instance.user_set.remove(*remove_users)

        remove_permissions = cleaned_data.get("remove_permissions")
        if remove_permissions:
            remove_keycloak_group_permission(instance.name, remove_permissions)
...
class PermissionGroupDelete(ModelDeleteMutation):
    ...
    def clean_instance(cls, info, instance):
        remove_keycloak_group(instance.name)
        requestor = info.context.user
        if requestor.is_superuser:
            return
...
def add_keycloak_group(name):
    try:
        if admin_connector.get_group_by_name(group_name='/' + name):
            return
...
def add_keycloak_group_permission(name, add_permissions):
    try:
        roles = []
        group_id = admin_connector.get_group_by_name(group_name='/' + name)['id']
        for permission in add_permissions:
            role = get_role_by_name(permission.codename)
            roles.append(role)
...
def add_keycloak_group_user(name, users):
    for user in users:
        try:
            group_id = admin_connector.get_group_by_name(group_name='/' + name)['id']
            user_id = admin_connector.get_user_id_by_username(user.get_username())
            admin_connector.group_user_add(user_id, group_id)
        except KeycloakError as e:
            raise e
...
def remove_keycloak_group_permission(name, add_permissions):
    try:
        roles = []
        group_id = admin_connector.get_group_by_name(group_name='/' + name)['id']
        for permission in add_permissions:
            role = get_role_by_name(permission.codename)
            roles.append(role)
        admin_connector.delete_group_realm_roles(group_id=group_id, roles=roles)
    except KeycloakError as e:
        raise e


def remove_keycloak_group_user(name, users):
    for user in users:
        try:
            group_id = admin_connector.get_group_by_name(group_name='/' + name)['id']
            user_id = admin_connector.get_user_id_by_username(user.get_username())
            admin_connector.group_user_remove(user_id, group_id)
        except KeycloakError as e:
            raise e


def remove_keycloak_group(name):
    try:
        group_id = admin_connector.get_group_by_name(group_name='/' + name)['id']
        admin_connector.delete_group(group_id)
    except KeycloakError as e:
        raise e
