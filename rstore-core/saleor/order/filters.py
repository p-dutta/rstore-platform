...
def filter_created_range(qs, _, value):
    start_date = value.get("gte", None)
    six_month = datetime.date.today() + relativedelta(months=-6)
    if start_date:
        if start_date < six_month:
            raise ValidationError(
                {
                    "start_date": ValidationError(
                        "Start date can not be more than 6 months old", code=OrderErrorCode.NOT_FOUND
                    )
                }
            )
