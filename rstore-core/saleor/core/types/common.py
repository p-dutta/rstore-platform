...
from ..enums import (
    ...
    PartnerErrorCode,
    RequestErrorCode,
    UserPermissionUpdateErrorCode,
    NotificationErrorCode,
    AnnouncementErrorCode,
    NotificationMetaErrorCode,
    SegmentErrorCode,
    TargetErrorCode,
    AchievementErrorCode,
    RuleErrorCode, UserProfileErrorCode,
    CommissionErrorCode,
    TargetPartnerSalesErrorCode, NoticeErrorCode, NoticeDocumentErrorCode
)
...
class RequestError(Error):
    code = RequestErrorCode(description="The error code.", required=True)
...
class PartnerError(Error):
    code = PartnerErrorCode(description="The error code.", required=True)


class NotificationError(Error):
    code = NotificationErrorCode(description="Notification error code.", required=True)


class AnnouncementError(Error):
    code = AnnouncementErrorCode(description="Announcement error code.", required=True)


class SegmentError(Error):
    code = SegmentErrorCode(description="Segment error code.", required=True)

class NotificationMetaError(Error):
    code = NotificationMetaErrorCode(description="notification meta error code.", required=True)

class UserPermissionUpdateError(Error):
    code = UserPermissionUpdateErrorCode(description="The user permission update error code.", required=True)


class TargetError(Error):
    code = TargetErrorCode(description="Target error code.", required=True)


class AchievementError(Error):
    code = AchievementErrorCode(description="Achievement error code.", required=True)


class RuleError(Error):
    code = RuleErrorCode(description="Rule error code.", required=True)


class UserProfileError(Error):
    code = UserProfileErrorCode(description="User Profile error code.", required=True)


class CommissionError(Error):
    code = CommissionErrorCode(description="Commission error code.", required=True)


class TargetPartnerSalesError(Error):
    code = TargetPartnerSalesErrorCode(description="The error code.", required=True)


class NoticeError(Error):
    code = NoticeErrorCode(description="Notice error code.", required=True)


class NoticeDocumentError(Error):
    code = NoticeDocumentErrorCode(description="Notice document error code.", required=True)
